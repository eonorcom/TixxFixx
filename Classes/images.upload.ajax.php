<?php
	function uploadImage($ID, $fileName, $maxSize, $maxW, $fullPath, $relPath, $colorR, $colorG, $colorB, $maxH = null){
		$folder = $_SERVER['DOCUMENT_ROOT']. "/Classes/images/cache/";
		$maxlimit = $maxSize;
	...
	$returnW = strip_tags($_REQUEST['returnW']);
	...
	if($filesize_image > 0){
		$upload_image = uploadImage($ID, $filename, $maxSize, $maxW, $fullPath, $relPath, $colorR, $colorG, $colorB, $maxH);
...
<?php
	if($imgUploaded){
		echo '<img src="/Classes/images.cache.php?id='.$ID.'" border="0" />';
	}else{
		echo 'Error(s) Found: ';
